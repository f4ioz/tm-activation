...
- zip archive: <https://github.com/f4ioz/tm-activation/raw/main/releases/tm-activation-1.6.2.zip>
- tar.gz archive: <https://github.com/f4ioz/tm-activation/raw/main/releases/tm-activation-1.6.2.tar.gz>
...
```bash
wget https://github.com/f4ioz/tm-activation/raw/main/releases/tm-activation-1.6.2.tar.gz
```
...
- **Operators area** (`/activation`), protected by a shared password: each
  ...
  - **Operator accounts (your choice)**: one password shared by everyone, or
    **one password per operator**, set at their first login (their callsign is
    enough). Approval of new accounts by an administrator is optional, and
    several operators can be **administrators**. Settings → Operator accounts.
  - **Schedule** of slots (who, when, band, mode), with a warning if two slots
    ...
    QSO is recorded, as the log may simply not be imported yet.
...
3. Copy the archive onto the Pi, from the PC:
   `scp tm-activation-1.6.2.tar.gz utilisateur@tm50abc.local:`
   (or download it straight onto the Pi with `wget`, see
   [Download](#download))
4. On the Pi:

   ```bash
   tar xzf tm-activation-1.6.2.tar.gz
   cd tm-activation-1.6.2
   sudo ./install.sh --lan
   ```
