...
- **Certificat pour les chasseurs** : celui qui retrouve ses QSO sur la page
  publique repart avec un **certificat PDF** en A4 paysage — ses contacts, ses
  points, sa place au classement, le logo du club et le nom de l'activation.
  L'indicatif spécial y tient la vedette, souligné de son propre morse. Ouvert
  ou non au public d'un clic dans les Réglages ; le nom du chasseur n'y figure
  que si l'administrateur le demande, et le nombre de contacts listés comme la
  présence du journal en annexe se règlent.
...
| Planning des créneaux | Réglages : comptes et affichage public |
|---|---|
| ![Planning des créneaux](docs/images/planning.png) | ![Réglages : comptes opérateurs et affichage public](docs/images/settings.png) |
...
- Archive zip : <https://github.com/f4ioz/tm-activation/raw/main/releases/tm-activation-1.40.1.zip>
- Archive tar.gz : <https://github.com/f4ioz/tm-activation/raw/main/releases/tm-activation-1.40.1.tar.gz>
...
```bash
wget https://github.com/f4ioz/tm-activation/raw/main/releases/tm-activation-1.40.1.tar.gz
```
...
- **Espace opérateurs** (`/activation`), protégé par un mot de passe commun :
  ...
  - **Comptes opérateurs (au choix)** : mot de passe commun à tous, ou **un mot
    ...
    Les exigences du mot de passe se règlent (longueur, majuscules, chiffres,
...
| Exigences du mot de passe | 8 car., 1 maj., 1 chiffre, 1 spécial | chaque seuil réglable (0 = pas d'exigence) |
...
| QR code sur le certificat | aucun | adresse au choix (site du club, page de l'activation…), imprimée aussi en clair |
...
3. Copier l'archive sur le Pi, depuis le PC :
   `scp tm-activation-1.40.1.tar.gz utilisateur@tm50abc.local:`
   (ou la télécharger directement sur le Pi avec `wget`, voir
   [Téléchargement](#téléchargement))
4. Sur le Pi :

   ```bash
   tar xzf tm-activation-1.40.1.tar.gz
   cd tm-activation-1.40.1
   sudo ./install.sh --lan
   ```
   ...
   ```bash
   unzip tm-activation-1.40.1.zip
   cd tm-activation-1.40.1
...
Le moteur des certificats (`app/tmcert/`, reportlab) est autonome : format des
données, mise en page et ligne de commande sont décrits dans
`app/tmcert/SPEC.md`. Les textes des pages se traduisent dans
`app/locales/<langue>.json`.
