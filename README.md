...
- Archive zip : <https://github.com/f4ioz/tm-activation/raw/main/releases/tm-activation-1.17.1.zip>
- Archive tar.gz : <https://github.com/f4ioz/tm-activation/raw/main/releases/tm-activation-1.17.1.tar.gz>
...
```bash
wget https://github.com/f4ioz/tm-activation/raw/main/releases/tm-activation-1.17.1.tar.gz
```
...
- **Espace opérateurs** (`/activation`), protégé par un mot de passe commun :
  ...
  - **Suis-je spotté ?** : les derniers spots DX de l'indicatif (DXWatch, puis
    ...
    reprend dans le formulaire. Sans Internet, le panneau disparaît, sans erreur.
  - **Pays contactés** : les drapeaux des entités DXCC déjà travaillées
    ...
    réseau nécessaire, et l'affichage est le même sous Windows, Android ou Linux).
...
3. Copier l'archive sur le Pi, depuis le PC :
   `scp tm-activation-1.17.1.tar.gz utilisateur@tm50abc.local:`
   (ou la télécharger directement sur le Pi avec `wget`, voir
   [Téléchargement](#téléchargement))
4. Sur le Pi :

   ```bash
   tar xzf tm-activation-1.17.1.tar.gz
   cd tm-activation-1.17.1
   sudo ./install.sh --lan
   ```
   ...
   ```bash
   unzip tm-activation-1.17.1.zip
   cd tm-activation-1.17.1
